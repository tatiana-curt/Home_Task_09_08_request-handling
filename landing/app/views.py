...
def index(request):
    ...
    index = request.GET.get('from-landing')
    if index == 'original':
...
def stats(request):
    ...
    try:
        ...
        test_conversion = 'Недостаточно данных для расчета конверсии, проверьте счетчик - counter_show'
        original_conversion = 'Недостаточно данных для расчета конверсии, проверьте счетчик - counter_show'
